...
#Força bruta para A02 - Cryptographic Failures
def brute_force_cryptographic_failures():
    print("=" * 50)
    print("Força Bruta em A02 - Cryptographic Failures")
    print("=" * 50)

    results = 0
    test_protocols = ["http", "https"]
    for proto in test_protocols:
        url = f"{proto}://{NF_CONFIG['NRF_IP']}:{NF_CONFIG['NRF_PORT']}"
        try:
            r = requests.get(url, timeout=3, verify=False)
            if proto == "http":
                print(f"[!!!] HTTP inseguro detectado via força bruta em {url}")
                results += 1
...
#Força bruta para Segmentação de Rede (testar conexões TCP em múltiplas portas)
def brute_force_network_slicing():
    print("=" * 50)
    print("Força Bruta em Segmentação de Rede")
    print("=" * 50)

    success = 0
    #Testar portas num intervalo próximo da porta padrão
    for port in range(NF_CONFIG['UDM_PORT'], NF_CONFIG['UDM_PORT'] + 5):
        try:
            socket.create_connection((NF_CONFIG['UDM_IP'], port), timeout=2).close()
            print(f"[!!!] Conexão TCP aberta na porta {port} - potencial falha de segmentação")
            success += 1
        except:
            print(f"Porta {port} fechada ou inacessível.")
    return success
...
def gerar_grafico_linha(resultados):
    categorias = [
        ...
        "Criptografia Força Bruta", "Login inválido", "Endpoint inválido",
        "Conexão inter-slice", "Bypass autorização",
        "Segmentação Força Bruta"
    ]
...
if __name__ == "__main__":
    ...
    bf_crypt = brute_force_cryptographic_failures()
    bf_segment = brute_force_network_slicing()

    #Testes adicionais para ping e etc, pode incluir se quiser:
    ...
    resultados = [r1, r2, bf_crypt, 0, 0, r3, r4, bf_segment]
